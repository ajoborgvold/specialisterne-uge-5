...
def validate_data(file_path):
    ...
    Preliminary data handling and validation:
        1. Open file; if the file does not exist, handle this exception
        2. Read the rows:
...
def validate_row(row, row_id, validation_functions, patterns):
    ...
    # print(f"Row: {row_id}, content: {row}")
...
def check_file_permissions(file_path):
    """
    Permission check:
        1. Check if there are read, write and execute permissions for the data file
        2. Print the permissions status
        3. If read permissions are granted, call the validate_data function; if not, print a message
    """
